...
# reading the file and choosing the columns
...
# make and name the scatter plots
...
# this cleans up, saves, and displays the .png file
plt.tight_layout()
plt.savefig('automobile.png')
plt.show()

# this creates dummy variables for categorical data
df_encoded = pd.get_dummies(df, columns = ['fuel-type', 'body-style', 'aspiration'], drop_first = True)

# separates and X and y
...
for var, coef in zip(model.feature_names_in_, model.coef_):
    print(f'{var}: {coef:.2f}')
# your code for necessary preprocessing and model fitting
